...
export async function generateDetailedInsight(
  ...
  try {
    ...
    let systemPromptWithConstraints = `
${customizedPrompt}
...
## USER'S SPECIFIC REFINEMENT REQUIREMENTS

The user has provided additional context to refine their search:

${refinementContext.trim()}

**CRITICAL:** These refinement requirements are ADDITIONAL preferences on top of the base constraints above. Please incorporate these specific requirements into your analysis and product recommendations while still maintaining ALL the hard constraints listed above.`;
    }
...
${refinementContext ? '6. **Incorporate the user\'s refinement requirements above**' : ''}
...
`;
    ...
    // Build user prompt
    const baseUserPrompt = `Provide a comprehensive market analysis for the following trending product insight.
...
${grokInsight}`;
    ...
    const userPrompt = refinementContext
      ? `${baseUserPrompt}

**USER'S REFINEMENT REQUIREMENTS:**
${refinementContext.trim()}

Please ensure all recommendations align with both the system constraints AND the user's specific refinement requirements above.
...
- **Address the user's specific refinement requirements stated above**`
      : `${baseUserPrompt}
...
- Tailor all insights specifically for the ${region} market`;
    ...
    console.log(`- Refinement Context: ${refinementContext ? 'Yes' : 'No'}`);
    console.log(`- Live Search: ENABLED`); // ← NEW

    // ============================================
    // CRITICAL CHANGE: ADD SEARCH_PARAMETERS
    // ============================================
  ...
  "https://api.x.ai/v1/chat/completions",
  {
    ...
    search_parameters: {
      ...
      return_citations: true,         // Get source URLs for transparency
      max_search_results: 6,         // Increased from 5 to 10 for more suppliers
      sources: [
        {
          type: "web",
          // Only search these trusted B2B manufacturer platforms
          allowed_websites: [
            "globalsources.com",      // Premium verified suppliers
            "alibaba.com",            // Largest B2B marketplace
            "tradekey.com",           // International B2B
            "ec21.com",               // Korea-based platform
            "thomasnet.com"           // North American suppliers
          ],
          safe_search: false          // Allow all business results
        },
        {
          type: "news"                // Include recent market/trend news
        }
      ]
    }
  },
  {
    ...
    // ← NEW: Extract citations if available
    const citations = data.choices?.[0]?.message?.citations || [];
    const numSourcesUsed = data.usage?.num_sources_used || 0;

    console.log("✅ Grok response received, length:", detailedInsight.length);
    console.log(`🔍 Live Search Stats:`);
    console.log(`   - Sources used: ${numSourcesUsed}`);
    console.log(`   - Citations found: ${citations.length}`);
    console.log(`   - Cost: $${(numSourcesUsed * 0.025).toFixed(3)}`);
    ...
    return {
      ...
      // ← NEW: Include search metadata
      searchMetadata: {
        sourcesUsed: numSourcesUsed,
        citations: citations,
        searchCost: numSourcesUsed * 0.025
      }
    };
  } catch (error) {

...
export const faqController = {
      ...
      res
        ...
        .json(
          new ApiResponse(
            200,
            data,
            "Successfuly retreived all the Faqs for admin"
          )
        );
    } catch (error) {
      console.error(error);
        ...
        return res
          ...
          .json(
            new ApiResponse(
              400,
              null,
              "Category, question and answer are required"
            )
          );
      }
  ...
  async updateFaqs(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, updated, "FAQ updated successfully"));
    } catch (error) {
      if (error.code === "NO_FIELDS") {
        return res
          .status(400)
          .json(new ApiResponse(400, null, "No fields provided to update"));
      }
      next(error);
    }
  },
